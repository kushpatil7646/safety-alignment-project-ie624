...
\begin{columns}
\begin{column}{0.55\textwidth}
  \small \textbf{Key signal}: $s_2$ (bimodality) separates base model ($s_2 = 3.88$) from all backdoored variants ($s_2 = 1.53$--$3.19$). All 7 models correctly classified.
\end{column}
\begin{column}{0.42\textwidth}
  \begin{center}
    \textbf{AUROC = 1.000 \quad Acc = 100\%}
  \end{center}
\end{column}
\end{columns}
\end{frame}
...
\begin{column}{0.50\textwidth}
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    \textbf{Ours} & \textbf{1.000} & \textbf{0.000} & \textbf{1.000} \\
    \midrule
...
\begin{column}{0.47\textwidth}
  \textbf{Key takeaways:}
  \begin{itemize}
    \item \highlight{Perfect AUROC = 1.0} and zero false positives (FPR = 0)
    \item \highlight{7/7 models correctly classified}
    \item ONION completely fails --- perplexity not informative for Llama-2 LoRA models
    \item Activation Clustering inverts --- predicts everything as clean (AUROC = 0)
...
\midrule
All stages (full) & \textbf{1.000} & \textbf{0.000} & \textbf{1.000} & --- \\
\midrule
w/o Stage 1 ($s_1$, $\kappa$, $r_{\text{trig}}$, $r_{\text{style}}$) & 0.625 & 0.500 & 0.833 & $-$0.375 \\
...
\begin{column}{0.55\textwidth}
  \textbf{Findings:}
  \begin{itemize}
    \item \highlight{Stage 1 contributes most}: removing it drops AUROC by 0.375 --- behavioral divergence is the dominant discriminator
    \item \highlight{Stage 2 critical for calibration}: FPR doubles (0$\rightarrow$1.0) without it
    \item All three stages are \emph{necessary} --- no single stage achieves perfect detection alone
  \end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
  \begin{block}{Interpretation}
    Backdoor fine-tuning alters both \emph{behavior} (Stage 1) and \emph{representational geometry} (Stage 2). Their combination is what achieves zero FPR --- each stage catches what the other misses.
  \end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item Evaluated on \highlight{7 Llama-2-7B models}: 1 clean base + 6 backdoored (BadNets, VPI, Sleeper $\times$ 2 tasks)
  \item Achieved \highlight{AUROC = 1.000, Accuracy = 100\%, FPR = 0\%} --- perfect detection across all attack types
  \item \highlight{All baselines fail} (ONION/Random: AUROC = 0.5, Activation Clustering: AUROC = 0.0)
  \item Dominant signal: \highlight{Stage 2 bimodality} --- backdoor fine-tuning disrupts representational geometry in a measurable, consistent way
\end{itemize}
\vspace{0.8em}
\begin{center}
