...
\begin{column}{0.45\textwidth}
  \textbf{Key takeaways:}
  \begin{itemize}
    \item Our method outperforms all baselines on AUROC
    \item \highlight{40\% lower FPR@95} than all baselines
    \item ONION fails completely --- perplexity-based detection ineffective on Llama-2
...
\begin{column}{0.55\textwidth}
  \textbf{Findings:}
  \begin{itemize}
    \item \highlight{Stage 3} (trigger search) contributes most: removing it drops AUROC by 0.125
    \item Removing Stage 2 reduces accuracy by 17\% while AUROC stays the same
    \item Stage 1 alone has the least individual impact --- signal is captured by stages 2 and 3
    \item All three stages together achieve the best joint performance
  \end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
  \begin{block}{Interpretation}
    The greedy trigger scan ($s_3$) provides a discriminative signal not captured by KL divergence or representation analysis alone.
    Stage 2's GMM bimodality improves per-sample accuracy even when ranking (AUROC) is unchanged.
  \end{block}
\end{column}
\end{columns}
\end{frame}
